...
class Program
{
    ...
    static void Main(string[] args)
    {
       #region Transaction Nedir?
        ...
        //await context.SaveChangesAsync(); //*bu veriyi veritabanına kaydedeceksem eğer savecahngesasync yapmam yeterli değil çünkü yukarıda transactionu manule olarak kontrol ediyorum transaction rolback veya commit etmedim şuan kayıt işlemi yapmaz aşşağıda transactionu commit yaptığım zaman kaydedecek
        ...
        //await transaction.CreateSavepointAsync("t1");//*geridönüş noktası oluşturduk ve adına t1 dedik

        //Person p10 = await context.Persons.FindAsync(10);
        //context.Persons.Remove(p10);
        //await context.SaveChangesAsync();

        //await transaction.RollbackToSavepointAsync("t1");//*burada RollbackAsync dersek yapılan bütün işlemleri geri alır ama RollbackToSavepointAsync dersek vermiş olduğumuz geri dönüş noktasına kadar olanları geri alacaktır.
        //*yani burada ıd si 13 ve 11 olanlar silinir ama ıd si 10 olan silinmez
