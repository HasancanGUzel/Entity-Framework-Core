...
namespace Table_Per_Hierarchy_TPH_;
class Program
{
    ...
    static void Main(string[] args)
    {
       #region Table Per Hierarchy (TPH) Nedir?
        ...
        //*Yani bizim elimizde çalişan ve çalışmayan olarak 2 entity olduğunu varsayalım 
        /*
            Calisan          Calismayan
             -Id                -Id
             -Adi               -Adi
             -Soyadi            -Soyadi
             -X                 -Y
        *Şimdi bu entityler için veritabanında bizim 2 tane tablomuz olacak farkları sadece x ve y kolonları. bunun için TPH kullanabiliriz örneğin bu entitler için veritabanında ayrı ayrı tablo yerine ortak bir tablo olabilir.
        örneğin;

*        Id        Adi     Soyadi    X    Y    Discriminator
*         1       Hasan     sadasd   1          Calisan
*         2       ahmet     sadasd        3      Calismayan  

*       TPH DE böyle tek tabloda tutabiliriz.
        ...
        */
        ...
        //var employees = await context.Employees.ToListAsync(); //* burada technicianlarda gelir
        //var techs = await context.Technicians.ToListAsync(); //* burada ise sadece technician gelir
        ...
        //*Burada customerda a ve technicianda a prop var ve migration yaptığımız zaman bir tane A değişkeni bir tanede Technician_A diye değişken gözükecek normal A customer olurken  Technician_A adı üstünde technician
        //*peki neden böyle yapıyor oda kalıtım sırasına göre davranıyor customer personun çocuğuyklen  technician da personun torunu gibi gözüktüğü için
        #endregion
    }
}

    abstract class Person //abstract olmasada olur örnek için yaptık
    {
    ...
    class ApplicationDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            //    .HasDiscriminator<string>("ayirici") //*dicriminator kolonunu özellştiriyoruz string yerine int vs de yazabiliriz 
            //    .HasValue<Person>("A") //*burada ayirici(discrimnator )kolonunu özellştiriyoruz ve person eklenirse ayirici kolonunda A yazsın employee eklenirse B yazsın vs.   .hasvalue yazmasakta olur o zaman otomatik olarak employee eklersek veritabanına ayirici kolonuna employee hangi entityde gönderirsek o türe göre yazar.
            //    .HasValue<Employee>("B")
            //    .HasValue<Customer>("C")
            //    .HasValue<Technician>("D");

            //  modelBuilder.Entity<Person>().UseTphMappingStrategy(); //* efcore7 yeniliklerinde bunuda kullanabiliriz tph olduğunu belirtmek için tph de gerek yok default anlıyor zaten
        }
