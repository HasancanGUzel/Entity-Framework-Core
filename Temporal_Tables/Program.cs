...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        //Silindiği tarih
        // var dateOfDelete =  context.Persons.TemporalAll()
        //     .Where(p => p.Id == 3)
        //     .OrderByDescending(p => EF.Property<DateTime>(p, "PeriodEnd"))
        //     .Select(p => EF.Property<DateTime>(p, "PeriodEnd"))
        //     .First();

        // var deletedPerson =  context.Persons.TemporalAsOf(dateOfDelete.AddMilliseconds(-1))
        //     .FirstOrDefault(p => p.Id == 3);

        //  context.AddAsync(deletedPerson);

        //  context.Database.OpenConnectionAsync();

        //  context.Database.ExecuteSqlInterpolated($"SET IDENTITY_INSERT dbo.Persons ON");
        //  context.SaveChangesAsync();
        //  context.Database.ExecuteSqlInterpolated($"SET IDENTITY_INSERT dbo.Persons OFF");
...
class ApplicationDbContext : DbContext
{
    ...
    {
        // modelBuilder.Entity<Employee>().ToTable("Employees", builder => builder.IsTemporal()); //*bu tablonun temrporal table olduğunu belirledik.
